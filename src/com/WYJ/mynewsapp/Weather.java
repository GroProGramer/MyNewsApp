...
import com.WYJ.adapter.MyLIstViewAdapter;
import com.WYJ.util.UpdateWeather;
...
public class Weather extends Activity {
	...
	/**
	 * 主线程与更新天气的线程间通讯
	 * 本项目只有handle对象是该类实例，handle的handleMessage方法用于与UpdateWeather类
	 * 的onSuccess方法进行通讯
	 * 
	 */
	class MyHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case 1:
				handler.removeCallbacks(run);//删除指定的Runnable对象，使线程对象停止运行
	...
	private void initData() {
		...
		for (int i = 0; i < 4; i++) {
			if (dateArray[i].equals(currentWeekDay)) {
				index = i;//设置index为当前日子的在dateArray的下标
			}
		}
	}
	...
	private void updateWeatherInfo() {
		...
		weatherForecastList.setAdapter(new MyLIstViewAdapter(Weather.this,getData()));
	...
	private void updateWeather() {
		refreshing(true);//刷新进度条设置为可见
		handler.postDelayed(run, 60 * 1000);//定时操作
		...
		public void onClick(View v) {
			...
			case R.id.share:
				...
				intent.putExtra(Intent.EXTRA_TEXT,
						"我正在使用iWeather，可以随时随地查看天气信息，是您出差、旅行的贴心助手！推荐你也试试~，在360手机助手就可以下载。");
